function [img_files, pos, target_sz, ground_truth, video_path] = load_video_info(video_path)%解释完毕可封装
% 加载图片序列的信息
...
% schedule two
if exist([video_path num2str(frames{1}, 'img/%04i.png')], 'file'),%png格式
    video_path = [video_path 'img/'];
    img_files = num2str((frames{1} : frames{2})', '%04i.png');
elseif exist([video_path num2str(frames{1}, 'img/%04i.jpg')], 'file'),%jpg格式
    video_path = [video_path 'img/'];
    img_files = num2str((frames{1} : frames{2})', '%04i.jpg');
elseif exist([video_path num2str(frames{1}, 'img/%04i.bmp')], 'file'),%bmp格式
    video_path = [video_path 'img/'];
...
%list the files
img_files = cellstr(img_files);%将字符串类型的数组装换成细胞数组
